...
for dirname in ['INTERMEDIATE_DIR', 'PRODUCT_DIR', 'LIB_DIR', 'SHARED_LIB_DIR']:
    # Some gyp steps fail if these are empty(!), so we convert them to
    # variables
...
def GetCompilerPath(target_list, data, options):
    ...
    make_global_settings_dict = data[
...
def GenerateClasspathFile(target_list, target_dicts, toplevel_dir,
    ...
    AddElements('output', [os.path.join(
        toplevel_build, '.eclipse-java-build')])
        ...
        for action in target.get('actions', []):
            for input_ in action['inputs']:
                if (os.path.splitext(input_)[1] == '.java' and
                    ...
                    # package name if this proves to be too fragile in
                    # practice.
                    parent_search = dir_
                    while os.path.basename(parent_search) not in ['src', 'java']:
                        parent_search, _ = os.path.split(parent_search)
                        if not parent_search or parent_search == toplevel_dir:
                            # Didn't find a known root, just return the
                            # original path
                            yield dir_
                            break
                    else:
