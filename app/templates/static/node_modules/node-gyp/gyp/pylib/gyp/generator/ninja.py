...
class NinjaWriter(object):
    ...
    def WriteSpec(self, spec, config_name, generator_flags):
        ...
        if sources:
            ...
            link_deps = self.WriteSources(
                ...
                spec)
            # Some actions/rules output 'sources' that are already object
            # files.
            ...
            if obj_outputs:
                if self.flavor != 'mac' or len(self.archs) == 1:
                    ...
                    print "Warning: Actions/rules writing object files don't work with " \
                          "multiarch targets, dropping. (target %s)" % spec[
                              'target_name']
    ...
    def WriteRules(self, rules, extra_sources, prebuild,
        ...
        for rule in rules:
            ...
            # For each source file, write an edge that generates all the
            # outputs.
            for source in sources:
                ...
                # Gather the list of inputs and outputs, expanding $vars if
                # possible.
                ...
                if self.flavor == 'win':
                    # WriteNewNinjaRule uses unique_name for creating an rsp
                    # file on win.
    ...
    def WriteMacBundleResources(self, resources, bundle_depends):
        ...
        for output, res in gyp.xcode_emulation.GetMacBundleResources(
            ...
            if os.path.splitext(output)[-1] != '.xcassets':
                isBinary = self.xcode_settings.IsBinaryOutputFormat(
                    self.config_name)
    ...
    def WriteSources(self, ninja_file, config_name, config, sources, predepends,
        ...
        if self.flavor != 'mac' or len(self.archs) == 1:
            ...
            return dict((arch, self.WriteSourcesForArch(
                self.arch_subninjas[
                    arch], config_name, config, sources, predepends,
    ...
    def WriteSourcesForArch(self, ninja_file, config_name, config, sources,
        ...
        if self.flavor == 'mac':
            # Most targets use no precompiled headers, so only write these if
            # needed.
    ...
    def WritePchTargets(self, ninja_file, pch_commands):
        ...
        for gch, lang_flag, lang, input in pch_commands:
            ...
            cmd = map.get(lang)
            ninja_file.build(gch, cmd, input, variables=[
                             (var_name, lang_flag)])
    ...
    def WriteLinkForArch(self, ninja_file, spec, config_name, config,
        ...
        # linker flags are set via xcode_settings and msvs_settings,
        # respectively.
        ...
        if len(solibs):
            extra_bindings.append(
                ('solibs', gyp.common.EncodePOSIXShellList(solibs)))
    ...
    def WriteTarget(self, spec, config_name, config, link_deps, compile_deps):
        ...
        if spec['type'] == 'none' or (not link_deps and not extra_link_deps):
            ...
            # it doesn't do anything, and we fake out a 'binary' with a stamp
            # file.
            ...
            if (self.flavor not in ('mac', 'openbsd', 'netbsd', 'win') and not
                ...
                if self.flavor != 'mac' or len(self.archs) == 1:
                    ...
                    # call that combines single-arch .a files into a fat .a
                    # file.
                    self.AppendPostbuildVariable(variables, spec,
                                                 self.target.binary, self.target.binary)
    ...
    def GetSortedXcodePostbuildEnv(self):
        ...
        # TODO(thakis): It would be nice to have some general mechanism
        # instead.
    ...
    def WriteNewNinjaRule(self, name, args, description, is_cygwin, env, pool,
        ...
        if self.flavor == 'win':
            ...
            args = [gyp.xcode_emulation.ExpandEnvVars(
                arg, env) for arg in args]
...
def ComputeOutputDir(params):
    ...
    # Makes migrating from make to ninja easier, ninja doesn't put anything
    # here.
        ...
        if os.path.exists("/proc/meminfo"):
            with open("/proc/meminfo") as meminfo:
                ...
                for line in meminfo:
                    ...
                    if not match:
                        continue
                    # Allow 8Gb per link on Linux because Gold is quite memory
                    # hungry
...
def GenerateOutputForConfig(target_list, target_dicts, data, params,
    ...
    for qualified_target in target_list:
        ...
        this_make_global_settings = data[
            build_file].get('make_global_settings', [])
