    ...
    if 'targets' in data:
        ...
        for target in target_list:
            ...
            if 'toolsets' in target:
                del target['toolsets']
            if len(toolsets) > 0:
                # Optimization: only do copies if more than one toolset is
                # specified.
...
class ParallelState(object):
    ...
    def __init__(self):
        ...
        self.scheduled = set()
        # A list of dependency build file paths that haven't been scheduled
        # yet.
        self.dependencies = []
...
def ExpandVariables(input, phase, variables, build_file):
    ...
    # Convert all strings that are canonically-represented integers into
    # integers.
    if type(output) is list:
        ...
        for dependency_key in all_dependency_sections:
            ...
            for index in xrange(0, len(dependencies)):
                ...
                if not multiple_toolsets:
                    # Ignore toolset specification in the dependency if it is
                    # specified.
                    dep_toolset = toolset
...
def ExpandWildcardDependencies(targets, data):
    ...
    for target, target_dict in targets.iteritems():
        ...
        for dependency_key in dependency_sections:
            ...
            # because the dependencies list will be modified within the loop
            # body.
            index = 0
            while index < len(dependencies):
                ...
                # wildcard.
                dependency_target_dicts = data[
                    dependency_build_file]['targets']
...
def VerifyNoGYPFileCircularDependencies(targets):
    ...
    for target, spec in targets.iteritems():
        ...
        for dependency in target_dependencies:
            ...
            if not dependency_node:
                raise GypError("Dependancy '%s' not found" %
...
def MergeLists(to, fro, to_file, fro_file, is_paths=False, append=True):
    ...
    # If x is hashable, returns whether x is in s. Else returns whether x is
    # in l.
    ...
    for item in fro:
        ...
        if append:
            ...
            # This ensures that the earliest occurrence of the item will stay
            # put.
...
def MergeDicts(to, fro, to_file, fro_file):
    ...
    for k, v in fro.iteritems():
        ...
        if type(v) in (str, int):
            ...
            # Recurse, guaranteeing copies will be made of objects that require
            # it.
            if not k in to:
                to[k] = {}
    ...
    for (configuration, old_configuration_dict) in configs.iteritems():
        ...
        for (key, target_val) in target_dict.iteritems():
            ...
            if not key_base in non_configuration_keys:
                new_configuration_dict[
                    key] = gyp.simple_copy.deepcopy(target_val)
...
def ValidateActionsInTarget(target, target_dict, build_file):
    ...
    for action in actions:
        ...
        if action_command and not action_command[0]:
            raise GypError(
                "Empty action as command in target %s." % target_name)
...
def Load(build_files, variables, includes, depth, generator_input_info, check,
    ...
    # Apply exclude (!) and regex (/) list filters only for
    # dependency_sections.
    for target_name, target_dict in targets.iteritems():
        tmp_dict = {}
    ...
    for settings_type in ['all_dependent_settings',
                          'direct_dependent_settings',
                          'link_settings']:
        DoDependentSettings(settings_type, flat_list,
                            targets, dependency_nodes)
    ...
    # Apply "post"/"late"/"target" variable expansions and condition
    # evaluations.
    for target in flat_list:
        target_dict = targets[target]
        build_file = gyp.common.BuildFile(target)
