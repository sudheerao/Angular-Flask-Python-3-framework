...
def WriteRules(target_name, rules, extra_sources, extra_deps,
    ...
    for rule in rules:
        ...
        for count, rule_source in enumerate(rule.get('rule_sources', [])):
            ...
            # Create variables for the output, as 'local' variable will be
            # unset.
            these_outputs = []
...
def WriteTarget(namer, qualified_target, target_dicts, build_dir, config_to_use,
    ...
    # CMake unfortunately has no means of removing entries from
    # LINK_DIRECTORIES.
    ...
    if library_dirs is not None:
        ...
        for library_dir in library_dirs:
            output.write(' ')
            output.write(NormjoinPath(
                path_from_cmakelists_to_gyp, library_dir))
            output.write('\n')
        output.write(')\n')
    ...
    if target_type != 'none':
        ...
        if (not cflags_c or not c_sources) and (not cflags_cxx or not cxx_sources):
            ...
            # as other targets may require different properties on the same
            # files.
    ...
    # Give CMake as little information as possible so that it doesn't mess it
    # up.
    ...
    # extra_deps currently only depend on their own deps, so otherwise run
    # early
...
def PerformBuild(data, configurations, params):
    ...
    # generator_dir: relative path from pwd to where make puts build files.
    # Makes migrating from make to cmake easier, cmake doesn't put anything
    # here.
    generator_dir = os.path.relpath(options.generator_output or '.')
