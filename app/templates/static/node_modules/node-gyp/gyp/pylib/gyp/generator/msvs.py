...
def _GenerateExternalRules(rules, output_dir, spec,
    ...
    for rule in rules:
        trigger_files = _FindRuleTriggerFiles(rule, sources)
        for tf in trigger_files:
            # Get all the inputs and outputs for this rule for this trigger
            # file.
            inputs, outputs = _RuleInputsAndOutputs(rule, tf)
...
def _GenerateRulesForMSVS(p, output_dir, options, spec,
    ...
    rules_native = [r for r in rules if not int(
        r.get('msvs_external_rule', 0))]
    ...
    for cpy in spec.get('copies', []):
        for src in cpy.get('files', []):
            ...
            if src.endswith('/'):
                ...
                copies.append(
                    ([src], [dst], cmd, 'Copying %s to %s' % (src, dst)))
    return copies
...
def _InitNinjaFlavor(params, target_list, target_dicts):
    ...
    for qualified_target in target_list:
        ...
        if spec.get('msvs_external_builder'):
            # The spec explicitly defined an external builder, so don't change
            # it.
            continue
...
def CalculateVariables(default_variables, params):
    ...
    # Stash msvs_version for later (so we don't have to probe the system
    # twice).
                ...
                for config_name, configuration in spec['configurations'].iteritems():
                    ...
                    if precompiled_source != '':
                        ...
                        if not extensions_excluded_from_precompile:
                            ...
                            # not try to use it for C++ sources, and vice
                            # versa.
...
def _GenerateMSBuildProject(project, options, version, generator_flags):
    ...
    content += _GetMSBuildGlobalProperties(spec,
                                           project.guid, project_file_name)
